...
public sealed class GeoLogRepository : IGeoLogRepository
{
    ...
    {
        const string proc = "dbo.usp_GeoClientLog_Insert";

        try
        {
            await SqlGuard.EnsureOpenAsync(_dbSession.Connection, ct);
            await SqlGuard.EnsureStoredProcExistsAsync(_dbSession.Connection, proc, _dbSession.Transaction, ct);

            using var cmd = new SqlCommand(proc, _dbSession.Connection, _dbSession.Transaction)
            ...
            var langs = e?.Languages is { Length: > 0 } ? string.Join(",", e!.Languages!) : null;
